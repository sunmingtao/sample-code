...
def train(training_data):
    ...
    label_q_vals = rewards.reshape(-1, 1) * -500 * done.reshape(-1, 1) + (1 - done.reshape(-1, 1)) * discount_rate * np.amax(next_state_q_vals, axis=-1).reshape(-1, 1) # shape = (batch_size, 1)
