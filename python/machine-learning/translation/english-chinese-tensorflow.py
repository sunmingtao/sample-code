...
def pad_num_list(num_list, size):
    return [0] * (size - len(num_list)) + num_list
...
for epoch_i in range(epochs):
    ...
    saver.save(sess, "./english-chinese-translate")


saver.restore(sess, "./english-chinese-translate")
test_input_texts = input_texts[10000:10010]
X_test_batch = [input_text_to_x(input) for input in test_input_texts]
dec_input = np.zeros((len(X_test_batch), 1))
...
[output_num_to_text(num_list) for num_list in dec_input]
