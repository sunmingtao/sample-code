'''
Based on version 2.
Minor change. Same as version 2, run 10 epochs
'''
import os
import re
import collections
...
for epoch_i in range(epochs):
    ...
    for batch_i, (source_batch, target_batch) in enumerate(batch_generator(X_train, y_train)):
        ...
        if (batch_i+1) % 50 == 0:
            print('Epoch {}, Batch {:3} Loss: {:>6.3f} Accuracy: {:>6.4f} Epoch duration: {:>6.3f}s'.format(epoch_i, (batch_i+1), batch_loss, total_accuracy / (batch_i + 1), time.time() - start_time))
...
start, end = 1000, 2000
test_input_texts = input_texts[start:end]
test_output_texts = output_texts[start:end]
X_test_batch = [input_text_to_x(input) for input in test_input_texts]
y_test_batch = np.array([output_text_to_y(output) for output in test_output_texts])
...
def seq_accuracy(a, b):
    return np.mean(a == b, axis=-1)

for i in range(output_max_size-1):
...
total_bleu = 0
for eng, target_chi, pred_chi, accuracy in zip(test_input_texts, test_output_texts, [output_num_to_text(num_list) for num_list in dec_input], seq_accuracy(y_test_batch, dec_input)):
    bleu = calculate_bleu(target_chi, pred_chi)
    total_bleu += bleu
    print(eng, target_chi, pred_chi, accuracy, bleu)
print('avg accuracy is {}, avg bleu is {}'.format(np.mean(y_test_batch == dec_input), total_bleu / (end - start)))
...
logits = tf.placeholder(tf.float32, (1, 3, 2), 'logits')
targets = tf.placeholder(tf.int64, (1, 3), 'targets')

loss = tf.contrib.seq2seq.sequence_loss(logits, targets, tf.ones([1, 3]))

sess = tf.InteractiveSession()
sess.run(tf.global_variables_initializer())
logits_feed = np.array([[1,2],[3,4],[7,6]])
logits_feed = np.expand_dims(logits_feed, 0)
targets_feed = np.array([1,1,0])
targets_feed = np.expand_dims(targets_feed, 0)

feed_dict = {logits:logits_feed, targets:targets_feed}
