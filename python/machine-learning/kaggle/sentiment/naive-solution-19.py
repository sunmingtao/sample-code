'''Change log
Based on version 16, use pretrained embedding vector
'''
import numpy as np
...
embed_path = '/Users/msun/Documents/crawl-300d-2M.vec'
...
full_text = list(all_phrase_list) + list(test_df['Phrase'].values)
tk.fit_on_texts(full_text)
word_index = tk.word_index
nb_words = min(vocabulary_size, len(word_index))
embedding_matrix = np.zeros((nb_words + 1, n_neurons))
...
embedding_layer = Embedding(input_dim=embedding_matrix.shape[0], output_dim=n_neurons, weights=[embedding_matrix], trainable=False, name='embedding')
