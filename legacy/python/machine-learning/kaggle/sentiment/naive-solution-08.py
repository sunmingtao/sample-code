'''Change log
Based on version 3
Add another LSTM layer, does not improve
'''
import numpy as np
...
print('Build model...')
model = Sequential()
model.add(Embedding(vocabulary_size, n_neurons))
model.add(LSTM(n_neurons, dropout=0.4, recurrent_dropout=0.4, return_sequences=True))
model.add(LSTM(n_neurons//2, dropout=0.5, recurrent_dropout=0.5))
...
model.save('sentiment_model_008.h5')

model = models.load_model('sentiment_model_008.h5', compile=False)
...
sub_pd.to_csv('sentiment-008.csv', index=False)
