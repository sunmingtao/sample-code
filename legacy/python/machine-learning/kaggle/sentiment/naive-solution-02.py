...
with tf.Session() as sess:
    ...
    for epoch in range(n_epochs):
        train_data_generator = training_data_generator(train_set)
        val_data_generator = training_data_generator(validation_set, len(validation_set))
        total_steps = len(train_set) // BATCH_SIZE
        for i in range(total_steps):
            X_batch, y_batch, seq_length_batch = next(train_data_generator)
            ...
            if i % 200 == 0:
                ...
                print('Processed {}, accuracy: {}'.format(i, acc_train))
        X_val_batch, y_val_batch, seq_val_length_batch = next(val_data_generator)
        acc_val = accuracy.eval(feed_dict={X: X_val_batch, y: y_val_batch, seq_length: seq_val_length_batch})
        print("{:4d}  Train loss: {:.4f}, Validation accuracy: {:.2f}%".format(epoch, loss_val, 100 * acc_val))
        saver.save(sess, "./my_sentiment_classifier2")
...
sub_pd.to_csv('sentiment-002.csv', index=False)
