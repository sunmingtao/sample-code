...
train_depth_pd = pd.merge(train_pd, depth_pd)
no_salt_pd = train_depth_pd[train_depth_pd['rle_mask'].isnull()]
train_depth_pd['rle_mask'].shape
...
model.compile(loss='binary_crossentropy', optimizer=Adam(1e-4, decay=1e-6), metrics=[iou, true_positive_rate, f_score, precision, recall, my_binary_accuracy])
...
model.save('salt_model_002.h5')

model1 = models.load_model('salt_model_001.h5', compile=False)
...
sub.to_csv('salt-submission_000.csv', index=False)
