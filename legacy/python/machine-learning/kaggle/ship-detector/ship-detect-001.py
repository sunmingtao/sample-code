...
unique_img_ids = unique_img_ids[unique_img_ids['ImageId'] != '6384c3e78.jpg']
...
seg_model.compile(optimizer=Adam(1e-4, decay=1e-6), loss=dice_p_bce, metrics=[dice_coef, my_binary_accuracy, true_positive_rate, f_score, precision, recall])
...
train_path = os.listdir(train_image_dir)
print(len(train_path), 'train images found')
...
c_img = np.expand_dims(c_img, 0)/255.0
cur_seg = fullres_model.predict(c_img)[0]
for i in range(len(cur_seg)):
    for j in range(len(cur_seg[i])):
        if cur_seg[i][j] > 0.1:
            print(i,j, cur_seg[i][j])
cur_seg = binary_opening(cur_seg>0.5, np.expand_dims(disk(2), -1))
...
submission_df['counts'] = submission_df.apply(lambda c_row: c_row['counts'] if
                                    ...
                                    0, 1)
submission_df['counts'].unique()

has_ship_df = submission_df[submission_df['counts'] > 0]
