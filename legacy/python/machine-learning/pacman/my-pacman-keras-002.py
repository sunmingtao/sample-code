...
dense_layer2 = Dense(n_actions + 1, activation='linear', name='dense2')
dense_outputs2 = dense_layer2(dense_outputs1)
output_layer = Lambda(lambda a: K.expand_dims(a[:, 0], -1) + a[:, 1:] - K.mean(a[:, 1:], keepdims=True), output_shape=(n_actions,), name='output')
...
def compile(optimizer, metrics=[]):
    ...
    assert trainable_model.output_names[1] == 'output'
