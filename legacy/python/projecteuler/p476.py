...
def get_point_circle_center(a, b, c, r):
    pca = math.sqrt(((b + c - a) / 2) ** 2 + r ** 2)
    pcb = math.sqrt(((a + c - b) / 2) ** 2 + r ** 2)
    return pca, pcb

def get_new_sides(a, A):
    new_aa = a / math.cos(A/2)
    new_ab = new_aa
    new_ac = a * math.tan(A/2) * 2
    return new_aa, new_ab, new_ac

def get_circle_area(a,b,c):
    r1 = get_r(a, b, c)
    A, B, C = get_angles(a, b, c)
    pca, pcb = get_point_circle_center(a, b, c, r1)
    a2, b2, c2 = get_new_sides(pca - r1, A)
    r2 = get_r(a2, b2, c2)
    a3, b3, c3 = get_new_sides(pcb - r1, B)
    r3_1 = get_r(a3,b3,c3)
    a3, b3, c3 = get_new_sides(pca - r1 - 2 * r2, A)
    r3_2 = get_r(a3, b3, c3)
    r3 = max((r3_1, r3_2))
    area1 = get_area(r1)
    area2 = get_area(r2)
...
def get_all_triangles(n):
    ...
    for a in range(1, n):
        ...
        if a % 10 == 0:
            print('Processed',a)
    return all_triangles, total


all_triangles, total = get_all_triangles(1803)

total_area = 0
counter=0
for a, b, c in all_triangles:
    area = get_circle_area(a,b,c)
    total_area += area
    counter += 1
    if counter % 1000000 == 0:
        print('Processed {} out of {}'.format(counter, total))



print (total_area/total)

get_circle_area(1,2,2)

a,b,c = 2,3,3
p = (b ** 2 + c ** 2 - a ** 2) / (2 * b * c)

math.cos(math.acos(p)/2)
...
class MalfattiProblem():
    ...
    def solve(self, triangle):
        ...
        self.__debug('''    #2 circle radius: {second_radius} {next_triangle}'''.format(second_radius=second_radius,
    ...
    def __place_third_circle(self, sorted_triangle, next_triangle, next_radius, r):
        ...
        self.__debug(
            '''        #3 a_nested_radius possible: {a_nested_radius}'''.format(a_nested_radius=a_nested_radius))
        ...
        self.__debug(
            '''        #3 descartes_radius possible: {descartes_radius}'''.format(descartes_radius=descartes_radius))
            ...
            for b in range(a, n + 1 - a):
                ...
                for c in c_list:
                    ...
                    if iter_count % 10000 == 0:
                        self.LOGGER.info(
                            '''Found: {a}, {b}, {c}, {area}, {count}, {sum}'''.format(a=a, b=b, c=c, area=area,
                                                                                      count=count, sum=R_sum))
...
for a in range(1, n+1):
    for b in range(a, n+1):
        for c in range(b, n+1):
            if a + b > c and a + b <= n:
                print (mp.solve((a,b,c)))

config_log(level=logging.DEBUG)
print (mp.solve((2,3,3)))